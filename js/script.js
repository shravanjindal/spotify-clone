...
async function getSongs(dir) {
    ...
    for (let index = 0; index < tiles.length; index++) {
        if (tiles[index].href.endsWith(".mp3")) {
            songinfo.songNames.push(tiles[index].innerText);
...
async function playbarController() {
    songbuttons[1].onclick = () => {
        ...
        if (!last_played.paused) {
            last_played.pause()
            button.innerHTML = `<img src="./icons/music.svg"><p>${button.childNodes[1].innerText}</p><img src="./icons/play2.svg"></img>`
            playbtn.childNodes[0].src = "./icons/play2.svg"
        }
        else {
            last_played.play()
            button.innerHTML = `<img src="./icons/music.svg"><p>${button.childNodes[1].innerText}</p><img src="./icons/play.svg"></img>`
            playbtn.childNodes[0].src = "./icons/play.svg"
        }
    }
    // let clickTimer;
    ...
    songbuttons[0].addEventListener('dblclick', () => {
        ...
        buttons[buttons.length - 1].childNodes[2].src = "./icons/play2.svg";
        audios.push(audio);
        buttons.push(newElements[prev].childNodes[0]);
        buttons[buttons.length - 1].childNodes[2].src = "./icons/play.svg";
        audio.play();
        playbtn.childNodes[0].src = "./icons/play.svg";
    });
...
function playfromStart() {
    if (audios[audios.length - 1]) {
        ...
        buttons[buttons.length - 1].innerHTML = `<img src="./icons/music.svg"><p>${buttons[buttons.length - 1].childNodes[1].innerText}</p><img src="./icons/play.svg"></img>`;
...
async function main(songinfo) {
    ...
    for (const song of songinfo.songNames) {
        ...
        i++;
        button.innerHTML = `<img src="./icons/music.svg"><p>${song.replace(".mp3", "")}</p><img src="./icons/play2.svg"></img>`
